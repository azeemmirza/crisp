...
public class CharacterConstant extends Constants{
    public CharacterConstant(String input){
    ...
    private STATE b(char c){
        ...
        }else if(isCharDigitsSS(c) || isBS(c) || isEscapseSequence(c) || isCharSpecialEscapeSequence(c)){
    ...
    private STATE c(char c){
        STATE output;
        if(isEscapseSequence(c) || isCharSpecialEscapeSequence(c)){
            output = this.states.B;
    ...
    private STATE accept(char c){
        STATE output;
        if(isChar(c) || isBS(c) || isSpecialChar(c)){
            output = this.states.DEAD;
        }else output = this.states.DEAD;
        return output;
    }
    private STATE dead(char c){
        STATE output;
        if(isCharDigitsSS(c) || isEscapseSequence(c) || isBS(c)){
            output = this.states.DEAD;
        }else output = this.states.DEAD;
        return output;
    }
